...
- The always-included set differs in what is in it: lnpm adds `CHANGELOG*`, `CHANGES*` and `HISTORY*`, and does not automatically include the files named by `bin`. The file named by `main` is included, as it is for npm, and it is the one entry that also beats your own `.npmignore` and `.gitignore` — everything else in the set is exempt from the `files` whitelist only. It still loses to the built-in exclusion list, only ever selects the one path it names, and does nothing at all in a package with no `files` field; `docs/adr/0004` records the reasoning and the conflict with `docs/adr/0001` that it accepts. Where the set applies is not a divergence: it is matched at the package root only, as npm's is, so a root `README.md` ships whatever `files` says while a `docs/README.md` is left to `files` and the ignore rules like any other path
