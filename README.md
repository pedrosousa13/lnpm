...
- The exclusion lists are not npm's. lnpm additionally excludes `.env`/`.env.*`, ignore files, logs and `*.tgz` by default, and does not drop the lockfiles (`package-lock.json`, `yarn.lock`, `pnpm-lock.yaml`, `bun.lockb`) that npm not only ignores but refuses to publish at all. Those additions are the overridable kind, so a manifest naming one directly in `files`, or negating it in an ignore file, still publishes it — a divergence in both directions, since npm never ignores `.env` and so publishes `dist/.env` for `"files": ["dist"]` where lnpm does not
...
- A `files` entry does not expand a directory a *glob* matched into that directory's subtree, where npm does. `files: ["dist/*"]` selects `dist/index.js` and stops; npm reads the same entry as also matching the directory `dist/cli` and ships `dist/cli/index.js` with it. A bare `["*"]` is the same divergence at the package root: npm ships the whole tree, lnpm only the root's own files. A literal directory name is expanded on both — `["dist"]` selects everything under `dist` — so the gap is glob-matched directories specifically
