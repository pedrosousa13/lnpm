...
- Supports gitignore pattern syntax — root anchoring (`/dist`), directory patterns (`dist/`) and negation (`*.txt` followed by `!keep.txt`), with the last matching pattern deciding. `*` matches within one path segment and `**` spans zero or more of them, so `**/*.pem` excludes a key at any depth including the package root, and `src/**/*.key` excludes one at any depth under `src`
