...
- Case sensitivity splits by who wrote the rule. The built-in list folds case on every platform, because it is a guard rather than a preference and `.ENV` is the same file as `.env` on macOS and Windows; patterns you write in a `.npmignore` or `.gitignore` never fold, so `secret.txt` there does not exclude `SECRET.TXT`. git splits it the other way round — it folds ignore matching whenever `core.ignorecase` is set, which it is by default on macOS and Windows
...
- The always-included set differs in what is in it: lnpm adds `CHANGELOG*`, `CHANGES*` and `HISTORY*`, and does not automatically include the files named by `bin`. The file named by `main` is included, as it is for npm — `main: "lib/index.js"` alongside `files: ["dist"]` publishes `lib/index.js`, with or without a leading `./`. It is the one entry that also beats your own ignore patterns, because a package whose declared entry point was left out does not load at all, and `lnpm publish` would otherwise report success on a broken package. It does not beat the built-in exclusion list, though — naming `.env` or anything else from that list as your `main` still does not publish it, since that list is a guard rather than a preference. Everything else in the set is exempt from the `files` whitelist only — an ignore pattern still drops it. Only the exact path `main` names is force-included: a `main` pointing at a directory does not pull that directory in, and a `main` pointing outside the package selects nothing. With no `files` field none of this applies and your ignore patterns decide the whole tree, `main` included. Where the set applies is not a divergence: it is matched at the package root only, as npm's is, so a root `README.md` ships whatever `files` says while a `docs/README.md` is left to `files` and the ignore rules like any other path
