...
//
// The name is validated through the removal entry point, which waives five
// reservations — #325's leading dot, #326's Windows device names and trailing
// dot or space, and #327's uppercase letter and non-NFC spelling — none of which
// is a path check. A project linked before one of those rules can hold a package
// it now refuses — ".hidden-pkg", or "con" or "foo." linked on Linux, where both
// are ordinary directory names, or "MyPkg" — and refusing to unlink it would
// leave it with no supported way out. The name is passed through as given and
// never composed: an entry whose name is decomposed on disk is only reachable by
// its decomposed name. Every traversal check still applies: packageName is
// joined into .lnpm/{name} for a RemoveAll below.
...
// Unlink, which differ only in the five reservations the removal form waives
// (#325's leading dot, #326's Windows device names and trailing dot or space,
// #327's uppercase letter and non-NFC spelling) and share every path check. But nothing guarded their ancestors, and a
...
// points, so a link writes outside the project and an unlink deletes outside it.
//
