...
// Since #325 ValidatePackageName rejects a leading dot on either segment, so no
// package linked after that change can own one of these names — but nothing
// revalidates a .lnpm populated before it, and nothing stops a user putting
// something else dot-named in there either. Reclaiming a directory that is not
// ours would be a far worse bug than the leak this sweep exists to fix. Only the
// exact shape the constructors produce is matched: the prefix, then the
// lowercase hex of a uint64, optionally followed by the retired suffix.
