...
lnpm runs your `prepack` and `prepare` scripts before packing (`publish` also runs `prepublishOnly`), so if your build is wired into one of those, push rebuilds for you. A plain `build` script is not run automatically.
